...
eval_fewshot_cpt.py
--------------------
Few-shot inference experiment using the CPT adapter ONLY (no VQA fine-tuning).
Runs 0-shot and 3-shot configs and saves separate result JSONs per config.

Few-shot examples are defined directly in __main__ as a list of dicts:
    FEW_SHOT_EXAMPLES = [
        {"question": "...", "answer": "...", "image_path": "path/to/image.jpg"},
        ...
    ]
Each example turn includes the actual image, matching the format of the query turn.
0-shot simply passes an empty list.
"""
...
# ---------------------------------------------------------------------------
# Benchmark class — CPT-only, with few-shot support
# ---------------------------------------------------------------------------

class FewShotCPTBenchmark:
    """
    Evaluation benchmark for CPT-only adapter with configurable few-shot prompting.

    Few-shot examples are prepended as prior assistant turns (text only, no image)
    so the model sees:
        [user: Q1]  →  [assistant: A1]          # shot 1
        [user: Q2]  →  [assistant: A2]          # shot 2  …
        [user: <image> + query question]         # actual query
    """
    ...
    def __init__(
        ...
        n = len(few_shot_examples)
        print(f"✓ FewShotCPTBenchmark initialised ({n}-shot)")
    ...
    # ------------------------------------------------------------------
    # Message construction — few-shot turns injected here
    # ------------------------------------------------------------------

    def _build_messages(self, image: Image.Image, question: str) -> List[Dict]:
        """
        Constructs the full message list for apply_chat_template.

        Structure:
            system  : Sinhala task instruction
            user/assistant pairs : few-shot examples (image + question → answer)
            user    : actual query image + question
        """
        messages = [
            {
                "role": "system",
    ...
    def _generate_answer_and_nll(
        ...
        # NOTE: do NOT cast the whole inputs dict to bfloat16 — corrupts
        # integer input_ids.  Move to device first, then handle dtype
        # only for floating-point tensors inside generate/model.
        ...
        # Gemma-3 requires token_type_ids; processor may not produce them
        ...
        # token_type_ids must be integer — pass zeros, NOT bfloat16
...
# ===========================================================================
# Runner — load CPT adapter once, iterate over shot counts
# ===========================================================================
...
if __name__ == "__main__":
    ...
    CPT_ADAPTER_DIR = os.path.abspath("model/cpt_adapter_20260304_154620")
    ...
    # ── Few-shot examples — edit these three dicts ───────────────────────────
    # Pick examples that are representative of your test distribution.
    # Images must NOT appear in your test set.
    FEW_SHOT_EXAMPLES = [
        {
        ...
        {
            "question":   "ඒ මනුස්සයා බයිසිකලේ උඩ මොකක්ද කරන්නේ?",
            "answer":     "ඔළුවෙන් හිටගෙන ඉන්නවා.",
            "image_path": "data/few_shot/2393692.jpg",
        },
    ]

    # ── Configs: 0-shot uses empty list, 3-shot uses all three examples ──────
    CONFIGS = {
        "0shot": [],
        "3shot": FEW_SHOT_EXAMPLES,
    }

    # ── Load model ONCE (shared across both configs) ─────────────────────────
